...
{
#ifdef DEBUG
	Serial.print("enter addPin. pin given: "); Serial.print(arduinoPin, DEC), Serial.print (" pin stored: ");
#endif
	PCintPin* tmp;
...
int8_t PCintPort::attachInterrupt(uint8_t arduinoPin, CallBackInterface* cbIface, int mode)
{
	...
	if ((portNum == NOT_A_PORT) || (cbIface == NULL)) 
	{
#ifdef DEBUG
	Serial.print("NOT_A_PORT or cbIface == NULL"); Serial.println(portNum, DEC);
#endif
		return(-1);
	}
